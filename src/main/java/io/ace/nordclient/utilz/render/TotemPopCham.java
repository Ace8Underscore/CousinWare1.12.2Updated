...
import io.ace.nordclient.hacks.render.PopCham;
...
public class TotemPopCham {
    ...
    public void onRenderWorld(final RenderWorldLastEvent event) {
        ...
        Color lineColorS = new Color(PopCham.rL.getValInt(), PopCham.bL.getValInt(), PopCham.gL.getValInt(), PopCham.aL.getValInt());
        Color fillColorS = new Color(PopCham.rF.getValInt(), PopCham.bF.getValInt(), PopCham.gF.getValInt(),PopCham. aF.getValInt());
        int lineA = lineColorS.getAlpha();
        int fillA = (fillColorS).getAlpha();
        final long time = System.currentTimeMillis() - this.startTime - ((Number)PopCham.fadestart.getValInt()).longValue();
        if (System.currentTimeMillis() - this.startTime > ((Number)PopCham.fadestart.getValInt()).longValue()) {
            double normal = this.normalize((double)time, 0.0, ((Number)PopCham.fadetime.getValDouble()).doubleValue());
        ...
        if (this.player != null && this.playerModel != null) {
            ...
            GL11.glEnable(2881);
            GL11.glEnable(2848);
            if (alphaFill > 1) alphaFill -= PopCham.fadetime.getValDouble();
            Color fillFinal = new Color(fillColor.getRed(), fillColor.getGreen(), fillColor.getBlue(), (int)alphaFill);

            if (alphaLine > 1) alphaLine -= PopCham.fadetime.getValDouble();
